...
public class TestDriver {
    ...
    public static final String ANSI_RESET = "\u001B[0m"; //no formatting after this

    public static void main(String[] args) {
        final long startTime = System.currentTimeMillis();
        final LauncherDiscoveryRequest request =
                LauncherDiscoveryRequestBuilder.request()
                        .selectors(selectClass(TrieTest.class), selectClass(SpellTest.class)) //selects which test classes to run
                        .build();
        ...
        launcher.execute(request); //launches test driver

        TestExecutionSummary summary = listener.getSummary();
        long testFoundCount = summary.getTestsFoundCount();
        List<Failure> failures = summary.getFailures(); //get the failed tests
        ...
        for(Failure failure : failures){ //Print each failure
            ...
            if(i != failure.getException().getStackTrace().length){
                message.append(" - Test File: ")
                        .append(failure.getException().getStackTrace()[i].getFileName()); //add test file that failure originated from to message
                message.append(" Line: ")
                        .append(failure.getException().getStackTrace()[i].getLineNumber()); //add line number of failure to message
            }
